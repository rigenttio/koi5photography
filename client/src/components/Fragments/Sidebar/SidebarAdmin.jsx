...
const SidebarAdmin = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-50 flex h-screen w-72 flex-col overflow-y-hidden bg-[#1C2434] duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 px-6 py-6">
        <div className="flex gap-3 items-center">
          ...
          <span className="font-bold text-2xl">Dashboard Admin</span>
        </div>
      ...
      <div className="flex flex-col overflow-y-auto duration-300 ease-linear">
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          {/* <!-- Menu Group --> */}
          <div>
            <h3 className="mb-4 ml-4 text-sm font-semibold text-slate-500">MENU</h3>

            <ul className="mb-6 flex flex-col gap-1.5">
              <li>
                <NavLink to="/admin/dashboard" className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium duration-300 ease-in-out hover:bg-slate-500/20 ${pathname.includes("dashboard") && "bg-slate-500/20"}`}>
              ...
              <li>
                <NavLink to="/admin/products" className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium duration-300 ease-in-out hover:bg-slate-500/20 ${pathname.includes("products") && "bg-slate-500/20"}`}>
